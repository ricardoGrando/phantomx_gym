...
from gym_phantomx.envs.mytf import euler_from_quaternion
from gym_phantomx.envs import Respawn
...
class phantomxEnv(gym.Env):
    ...
    def step(self, action):
        ...
        while data is None:
            try:
                if self.test_real:
                    data = rospy.wait_for_message('/phantomx/scan'+TURTLE, LaserScan, timeout=5)
                else:
                    data = rospy.wait_for_message('/phantomx/scan', LaserScan, timeout=5)
            except Exception:
                pass
